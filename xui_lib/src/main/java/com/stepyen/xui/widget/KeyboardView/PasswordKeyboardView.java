package com.stepyen.xui.widget.KeyboardView;
...
public class PasswordKeyboardView extends KeyboardView implements
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PasswordKeyboardView,
                defStyleAttr, 0);
        mDeleteDrawable = a.getDrawable(R.styleable.PasswordKeyboardView_pkv_deleteDrawable);
        mDeleteBackgroundColor = a.getColor(
                R.styleable.PasswordKeyboardView_pkv_deleteBackgroundColor, Color.TRANSPARENT);
        mDeleteWidth = a.getDimensionPixelOffset(R.styleable.PasswordKeyboardView_pkv_deleteWidth,
                -1);
        mDeleteHeight = a.getDimensionPixelOffset(R.styleable.PasswordKeyboardView_pkv_deleteHeight,
                -1);
        a.recycle();
    ...
    private void drawKeyBackground(Keyboard.Key key, Canvas canvas, int color) {
        ...
        drawable.setBounds(left, top, left + key.width, top + key.height);
    ...
    private void drawDeleteButton(Keyboard.Key key, Canvas canvas) {
        ...
        if (mDeleteDrawRect == null || mDeleteDrawRect.isEmpty()) {
            ...
            // 获取删除图标绘制的坐标
            int left = key.x + getPaddingLeft()+ (key.width - drawWidth) / 2 ;
            int top = key.y + getPaddingTop()+ (key.height - drawHeight) / 2 ;
