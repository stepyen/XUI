...
public class RadiusImageView extends AppCompatImageView {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        float scaleX = width / bmWidth; // 控件的宽 / bitmap的宽
        float scaleY = height / bmHeight;

        /*长或者宽是自适应大小时进行处理，根据Bitmap的大小，算出与控件的比例进行设置*/
        // 长和宽都是自适应大小
        ...
        } else if (widthWrapContent) {  // 宽是自适应大小,长是确定大小
            setMeasuredDimension((int) (bmWidth * scaleY), height);
        } else if (heightWrapContent) {// 宽是确定大小,长是自适应大小
            setMeasuredDimension(width, (int) (bmHeight * scaleX));
        }
    }
